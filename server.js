...
// ── Admin: Change promotor password ──────────────────────
app.put('/api/admin/promotors/:id/password', authAdmin, async (req, res) => {
  const { password } = req.body;
  if (!password || password.length < 6) return res.status(400).json({ error: 'Geslo mora imeti vsaj 6 znakov.' });
  const existing = query('SELECT id FROM promotors WHERE id = ?', [req.params.id]);
  if (!existing.length) return res.status(404).json({ error: 'Promotor ne obstaja.' });
  const hash = await bcrypt.hash(password, 10);
  run('UPDATE promotors SET password_hash = ? WHERE id = ?', [hash, req.params.id]);
  res.json({ success: true });
});

// ── Promotor: Change own password ────────────────────────
app.put('/api/auth/promotor/password', authPromotor, async (req, res) => {
  const { current_password, new_password } = req.body;
  if (!current_password || !new_password) return res.status(400).json({ error: 'Manjkajoči podatki.' });
  if (new_password.length < 6) return res.status(400).json({ error: 'Novo geslo mora imeti vsaj 6 znakov.' });
  const rows = query('SELECT * FROM promotors WHERE id = ?', [req.user.id]);
  if (!rows.length) return res.status(404).json({ error: 'Uporabnik ne obstaja.' });
  const valid = await bcrypt.compare(current_password, rows[0].password_hash);
  if (!valid) return res.status(401).json({ error: 'Trenutno geslo ni pravilno.' });
  const hash = await bcrypt.hash(new_password, 10);
  run('UPDATE promotors SET password_hash = ? WHERE id = ?', [hash, req.user.id]);
  res.json({ success: true });
});
